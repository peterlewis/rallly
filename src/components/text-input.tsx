...
export const TextInput = React.forwardRef<HTMLInputElement, TextInputProps>(
  function TextInput(
    ...
    return (
      <span
        className={clsx(
          "flex items-center rounded border bg-white text-slate-700",
          {
            "h-9 px-2": size === "md",
            "h-12 px-3": size === "lg",
            "input-error": error,
      ...
      >
        <input
          ...
          type="text"
          className={clsx(
            "h-full w-full appearance-none border-0 bg-transparent p-0 placeholder:text-slate-400 focus:outline-none focus:ring-0",
            {
              "text-xl": size === "lg",
            },
            className,
